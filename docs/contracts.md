...
| 20 | SDK install | [`contracts/sdk-install.md`](./contracts/sdk-install.md) | Per-language installer modules (Node + Python in MVP). Plan/apply decoupled. Manifests touched, lockfiles never. Node apply phase writes generated `otel-init` + injects entry-point require/import + per-package `.env.neat` with `OTEL_SERVICE_NAME` (ADR-047 + ADR-069) | ✅ landed (apply-side maturation v0.3.6) |
